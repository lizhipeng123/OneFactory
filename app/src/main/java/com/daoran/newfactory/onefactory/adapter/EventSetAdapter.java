...
import com.daoran.newfactory.onefactory.util.task.eventset.RemoveEventSetTask;
import com.daoran.newfactory.onefactory.view.dialog.ConfirmDialog;
import com.daoran.newfactory.onefactory.view.weight.SlideDeleteView;
...
public class EventSetAdapter extends RecyclerView.Adapter<EventSetAdapter.EventSetViewHolder> {
    ...
    public EventSetViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new EventSetViewHolder(LayoutInflater.from(mContext)
                .inflate(R.layout.item_event_set, parent, false));
    }
    ...
    private void gotoEventSetFragment(EventSet eventSet) {
        if (mContext instanceof MonthActivity) {
            ((MonthActivity) mContext).gotoEventSetFragment(eventSet);
        }
    }
